...
private:
    ...
    std::unordered_set<int64_t> m_generatingTerrain;
    std::unordered_map<int64_t, int> m_generatedTerrainBuffer;
    ...
    std::vector<Chunk*> chonksInit;
    std::vector<uPtr<VBOData>> chonksData;
    QMutex chonksInitmut;
    QMutex vboDatamut;
public:
    ...
    void createBigChungusAt(int x, int z);

    // Do these world-space coordinates lie within
    // a Chunk that exists?
    bool hasChunkAt(int x, int z) const;
    bool hasBigChungusAt(int x, int z);
    bool hasProcessedChungusAt(int x, int z);
    ...
    void draw(const glm::vec3& position, ShaderProgram *shaderProgram);

    // Check player position and add chunks if necessary
    void tick(const glm::vec3& position);
